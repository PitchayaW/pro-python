...
render_progress_sidebar(lesson_id="lesson06", total_exercises=13)
...
### ลำดับของ Argument (Positional และ Keyword)

การส่ง argument เข้าฟังก์ชันมี 2 แบบหลัก ๆ

**Positional argument** — ส่งค่าตามลำดับตำแหน่งที่นิยามไว้ ตรงไปตรงมาที่สุด

```python
def describe_person(name, age, city):
    print(f"{name} อายุ {age} ปี อยู่ที่ {city}")

describe_person("สมชาย", 25, "ขอนแก่น")   # ตำแหน่งต้องตรงกับที่นิยามไว้เป๊ะ
```

**Keyword argument** — ระบุชื่อ parameter ตรง ๆ ตอนเรียก ทำให้สลับลำดับการเขียนได้
และโค้ดอ่านเข้าใจง่ายขึ้นเมื่อมีหลาย parameter

```python
describe_person(city="ขอนแก่น", name="สมชาย", age=25)   # สลับลำดับได้ เพราะระบุชื่อ parameter ตรง ๆ
```

**กฎสำคัญเรื่องลำดับ:** ถ้าผสมทั้งสองแบบในการเรียกฟังก์ชันเดียวกัน
**positional argument ต้องมาก่อน keyword argument เสมอ** ไม่งั้นจะเกิด syntax error

```python
describe_person("สมชาย", age=25, city="ขอนแก่น")   # ถูก: positional (name) มาก่อน keyword
describe_person(name="สมชาย", 25, "ขอนแก่น")          # ผิด! SyntaxError
```

กฎเดียวกันนี้ใช้กับตอน**นิยาม**ฟังก์ชันด้วย — parameter ที่ไม่มีค่า default ต้องอยู่ก่อน
parameter ที่มีค่า default เสมอ

```python
def f(a, b=10):     # ถูก: a (ไม่มี default) อยู่ก่อน b (มี default)
    pass

def f(a=10, b):     # ผิด! SyntaxError: parameter ที่ไม่มี default ต้องมาก่อน
    pass
```

### *args และ **kwargs — รับ argument ได้ไม่จำกัดจำนวน

บางครั้งไม่รู้ล่วงหน้าว่าจะมีคนส่ง argument เข้ามากี่ตัว ไพธอนมีไวยากรณ์พิเศษรองรับ

`*args` รับ positional argument ได้ไม่จำกัดจำนวน รวบเป็น tuple ให้ใช้งาน
...
print(total(1, 2, 3))         # 6
print(total(1, 2, 3, 4, 5))    # 15  — ส่งกี่ตัวก็ได้
```

`**kwargs` รับ keyword argument ได้ไม่จำกัดจำนวน รวบเป็น dictionary ให้ใช้งาน
...
show_info(name="สมชาย", age=20)
# พิมพ์: name: สมชาย / age: 20
```

ผสมกันได้ทั้งหมดในฟังก์ชันเดียว โดยต้องเรียงลำดับ **parameter ปกติ → `*args` → `**kwargs`**
...
st.markdown("---")
st.markdown("### ข้อ 8: เติมโค้ดให้สมบูรณ์ — *args รับ argument ไม่จำกัด")
st.markdown("""
เติมโค้ดให้ฟังก์ชัน `multiply_all` ใช้ `*args` รับตัวเลขได้ไม่จำกัดจำนวน
แล้ว return ผลคูณของทุกตัว
""")

render_exercise(
    lesson_id="lesson06", exercise_id="ex8_fill_args",
    title="", instructions="",
    starter_code=(
        '# TODO: เติม *args ในวงเล็บของฟังก์ชัน\n'
        'def multiply_all():\n'
        '    result = 1\n'
        '    for n in args:\n'
        '        result = result * n\n'
    ...
    check_type="exact",
    expected_output="24",
    hint="เติม *args ในวงเล็บตอนนิยามฟังก์ชัน เป็น def multiply_all(*args): — args จะกลายเป็น "
         "tuple ของตัวเลขทุกตัวที่ส่งเข้ามา",
)

st.markdown("---")
st.markdown("### ข้อ 9: เติมโค้ดให้สมบูรณ์ — **kwargs รับ keyword argument ไม่จำกัด")
st.markdown("""
เติมโค้ดให้ฟังก์ชัน `print_profile` ใช้ `**kwargs` รับข้อมูลโปรไฟล์ได้ไม่จำกัดจำนวน
แล้วพิมพ์ทุก key-value ที่ได้รับ
""")

render_exercise(
    lesson_id="lesson06", exercise_id="ex9_fill_kwargsstar",
    title="", instructions="",
    starter_code=(
        '# TODO: เติม **kwargs ในวงเล็บของฟังก์ชัน\n'
        'def print_profile():\n'
        '    for key, value in kwargs.items():\n'
        '        print(f"{key}: {value}")\n'
        '\n'
        'print_profile(name="สมชาย", major="Computer Science")\n'
    ),
    check_type="exact",
    expected_output="name: สมชาย\nmajor: Computer Science",
    hint="เติม **kwargs ในวงเล็บตอนนิยามฟังก์ชัน เป็น def print_profile(**kwargs): — kwargs จะกลายเป็น "
         "dictionary ของทุก keyword argument ที่ส่งเข้ามา",
)
...
st.markdown("---")
st.markdown("### ข้อ 10: เขียนเอง — ฟังก์ชันแฟกทอเรียล")
st.markdown("""
...
render_exercise(
    lesson_id="lesson06", exercise_id="ex10_write_factorial",
    title="", instructions="",
...
st.markdown("---")
st.markdown("### ข้อ 11: เขียนเอง — ตรวจสอบ palindrome")
st.markdown("""
...
render_exercise(
    lesson_id="lesson06", exercise_id="ex11_write_palindrome",
    title="", instructions="",
...
st.markdown("---")
st.markdown("### ข้อ 12: เขียนเอง — ฟังก์ชันคำนวณค่าเฉลี่ยแบบมีเงื่อนไข")
st.markdown("""
...
render_exercise(
    lesson_id="lesson06", exercise_id="ex12_write_avgpassing",
    title="", instructions="",
...
st.markdown("---")
st.markdown("### ข้อ 13: เขียนเอง — ฟังก์ชันสร้างรหัสผ่านง่าย ๆ")
st.markdown("""
...
render_exercise(
    lesson_id="lesson06", exercise_id="ex13_write_maskemail",
    title="", instructions="",
